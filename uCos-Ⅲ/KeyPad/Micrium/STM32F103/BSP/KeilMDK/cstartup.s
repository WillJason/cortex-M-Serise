...
;                EXPORT  App_MemFault_ISR             [WEAK]
				IMPORT  App_MemFault_ISR_c
				TST     LR, #4
				ITE 	EQ
				MRSEQ	R0, MSP
				MRSNE 	R0, PSP
				B		App_MemFault_ISR_c
                B       .
                ENDP
App_BusFault_ISR\
                PROC
;                EXPORT  App_BusFault_ISR             [WEAK]
				IMPORT  App_BusFault_ISR_c
				TST     LR, #4
				ITE 	EQ
				MRSEQ	R0, MSP
				MRSNE 	R0, PSP
				B		App_BusFault_ISR_c
                B       .
                ENDP
App_UsageFault_ISR\
                PROC
;                EXPORT  App_UsageFault_ISR            [WEAK]
				IMPORT  App_UsageFault_ISR_c
				TST     LR, #4
				ITE 	EQ
				MRSEQ	R0, MSP
				MRSNE 	R0, PSP
				B		App_UsageFault_ISR_c
                B       .
                ENDP
App_Spurious_ISR\
                PROC
