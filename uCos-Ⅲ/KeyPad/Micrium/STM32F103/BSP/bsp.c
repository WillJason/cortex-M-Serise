...
#define  BSP_LED_START_BIT  (8 - 1)                            /* LEDs[3:1] are sequentially connected to PTD[10:8].  */
...
static  void  BSP_WIFI_Init  (void);
...
void  BSP_Init (void)
{
    ...
    RCC_DeInit();
    RCC_HSEConfig(RCC_HSE_Bypass);                                  /* HSE = 25MHz ext. crystal.                            */
...
* Description : .
*
* Argument(s) : count:	bytes to convert
*               inbuf:	source address  to be convert
*								outbuf:	result address  converted
* Return(s)   : number bytes of the converted result.
*
* Caller(s)   : Application.
*
* Note(s)     : none.
*********************************************************************************************************
*/
CPU_INT16U ConvertBytes7To8(CPU_INT16U count,const CPU_INT08U *inbuf,CPU_INT08U *outbuf) {    
	unsigned k = 0,i=0;    
	for (i = 0; i<count ; i++)    {        
		if ((i+1)%8==0) ++i;       
		if (i + 1 < count){           
			outbuf[k]=((inbuf[i]<<((i+1)%8))|((inbuf[i+1]<<1)>>(8-(i+1)%8)));
			++k;       
		}    
	}    
	return k; 
...
*********************************************************************************************************
*/
CPU_INT16U ConvertBytes8To7(CPU_INT16U count,const CPU_INT08U *inbuf,CPU_INT08U *outbuf) {    
	unsigned len,i,k = 0;    
	unsigned char  tH, tL, r;    
	for (i = 0; i < count; i++){        
		if (i % 7 == 0){           
			outbuf[k] = (inbuf[i] >>1);          
			k++;       
...
*                                             BSP_WIFI_Init()
*
* Description : Initialize the I/O for the WIFI module
*
* Argument(s) : none.
*
* Return(s)   : none.
...
static void BSP_WIFI_Init(void){
  // GPIO16 -- PC8 GPIO0 -- PC7  CH_PD -- PA8  GPIO2 -- PC9  
...
static  void  BSP_LED_Init (void)
{
    ...
    gpio_init.GPIO_Pin   = BSP_GPIOA_LED1|BSP_GPIOA_LED2;
...
void  BSP_LED_On (CPU_INT08U led)
{
    switch (led) {
        case 0:
             GPIO_ResetBits(GPIOA, (BSP_GPIOA_LED1|BSP_GPIOA_LED2));
             break;
...
void  BSP_LED_Off (CPU_INT08U led)
{
    switch (led) {
        case 0:
             GPIO_SetBits(GPIOA, (BSP_GPIOA_LED1|BSP_GPIOA_LED2));
             break;
...
void  BSP_LED_Toggle (CPU_INT08U led)
{
    ...
    switch (led) {
        case 0:
             ...
             pins ^= (BSP_GPIOA_LED1|BSP_GPIOA_LED2);
             GPIO_SetBits(  GPIOA,   pins  & (BSP_GPIOA_LED1|BSP_GPIOA_LED2));
             GPIO_ResetBits(GPIOA, (~pins) & (BSP_GPIOA_LED1|BSP_GPIOA_LED2));
             break;

        case 1:
					  pins = GPIO_ReadOutputData(GPIOA);
				    if ((pins & (1 << (led + 10))) == 0) {
                 GPIO_SetBits(  GPIOA, (1 << (led + 10)));
             } else {
                 GPIO_ResetBits(GPIOA, (1 << (led + 10)));
             }
            break;
        case 2:
					  pins = GPIO_ReadOutputData(GPIOA);
        ...
        case 3:
            ...
            if ((pins & (1 << (led + BSP_LED_START_BIT))) == 0) {
                 GPIO_SetBits(  GPIOA, (1 << (led + BSP_LED_START_BIT)));
             } else {
                 GPIO_ResetBits(GPIOA, (1 << (led + BSP_LED_START_BIT)));
             }
            break;
